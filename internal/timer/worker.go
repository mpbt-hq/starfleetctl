...
const (
	...
	// timerMsgPrefix marks directives generated by the timer worker, so the
	// receiving ship can tell a scheduled/automatic message apart from a
	// directive sent by another agent.
	timerMsgPrefix = "[timer] "
)
		...
		for _, t := range timers {
			...
			for _, target := range targets {
				...
				if t.Type == "command" {
					...
					_, err = bus.CommandFrom(t.Owner, target, verb, args)
				} else {
					// Directive (ship/user/control): post as message, attributed
					// to the timer's owning ship (not the worker) and marked as
					// a timer-generated message.
					err = bus.PostFrom(t.Owner, target, timerMsgPrefix+t.Text)
				}
				if err != nil {
					logf(logFile, "%s: tell %s failed: %v", t.ID, target, err)
				} else {
					logf(logFile, "%s: told %s [%s] as %s: %s", t.ID, target, t.Type, t.Owner, truncate(t.Text, 60))
				}
			}
			advanceOrDelete(store, t, persistentVarDir)
		}
