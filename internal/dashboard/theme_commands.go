...
func (d *Dashboard) DoReindex() error {
	...
	b.WriteString("Thin index — each row links to its own file under `dashboard/themes/`. Edit the theme\n")
	b.WriteString("file directly (Read/Edit as always); this index itself is regenerated with\n")
	b.WriteString("`scripts/starfleetctl dashboard reindex` and should not normally be hand-edited.\n\n")
