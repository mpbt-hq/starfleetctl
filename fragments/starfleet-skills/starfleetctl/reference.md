...
| `session attach --list` | List running detached sessions and the comms board in one call. |
...
- **`comms monitor-loop`/`fleet-watch` are known broken specifically when run under Claude
