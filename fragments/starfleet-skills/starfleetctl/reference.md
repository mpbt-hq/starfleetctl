...
| `session attach --list` | List running detached sessions and the agent-bus board in one call. |
| `session run <release> [--client claude\|opencode\|shell] [--name <id>] [--tier <tier>] [--supervisor <name>] [--permission-mode <mode>] [-- <args>]` | Launch a detached session for an agent/CLI and post the initial heartbeat (replaces the legacy `agent-run` helper). Pass `--print` to emit the shell-evaluable launch variables (`STARFLEET_SHIP_ID`, `SESSION`, `RELEASE_FULL`, `CLIENT`, `INNER_CMD`) instead. |
| `session stop <id\|session>` | Kill a detached session, clear its agent-bus heartbeat, and release its ship name (replaces the legacy `agent-run --stop` invocation). |
