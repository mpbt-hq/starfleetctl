...
1. **One allowlist entry covers every subcommand.** Tools like Claude Code gate shell commands
   ...
   `Bash(.starfleet-ai/bin/starfleetctl)`/`Bash(.starfleet-ai/bin/starfleetctl *)` pair covers every subcommand this
...
| Subcommand | Purpose |
|---|---|
| `agent-bus <cmd>` | Cross-session status board + directive bus. Worker side: `status <state> ["note"]`, `status <state> --task T --progress N --branch B --eta D --blocker X` (rich detail → `status/<ship>.json`), `inbox [--json]`, `ack <id>`, `ask "<q>"`, `clear`. Control side: `board [--json]`, `tell <agent> <text> [--reply <id>] [--stdin]`, `broadcast <text> [--reply <id>] [--stdin]`, `reply <qid> <answer>`, `asks`, `msgs [--json]`, `events [N]`, `prune`. `--json` on `board`/`inbox`/`msgs`/`asks` prints a JSON array instead of the human table; messages carry a `reply_to` field (In-Reply-To marker). `tell`/`broadcast` also accept `--stdin` to read the message body from stdin, bypassing the OS `ARG_MAX` limit on argv (use it for payloads > ~100 KB). Also has `monitor-loop`/`fleet-watch`/`watch` polling loops — **see [Known limitations](#known-limitations-and-parity-notes)**, they are not wired into any production polling harness. |
| `dashboard <cmd>` | Read/write/commit cycle for `mpbt-workspace`'s `DASHBOARD.md` (the thin, regenerated index): `pull`, `show`, `write <file\|->`, `commit -m "<msg>" [--no-push]`, `reindex` (rebuild the index from every `dashboard/topics/*.md` file's frontmatter). `dashboard topic <cmd>` is the per-topic-file counterpart, the only sanctioned way to read/write `dashboard/topics/*.md` (agents must not touch it via `Read`/`Edit`/`Write` directly — see `mpbt-workspace`'s `.starfleet-ai/agents.d/index.md`): `topic list [--json] [--category active\|parked] [--status <substr>]` (filter by category and/or status substring), `topic show <slug>`, `topic write <slug> <file\|->`, `topic new <slug> --title "<t>" [--status "<s>"] [--parked]`, `topic commit <slug> -m "<msg>" [--no-push]` (commits+pushes just that one file, correctly `git add`s it whether already tracked or brand new). |
...
| `session attach <id> [--read-only] [--independent]` | Resolve an agent ID / handle / tmux session name / unique substring to a running tmux session and attach the caller's terminal to it (shared rw / read-only / independent grouped window). Replaces the legacy `agent-attach` helper. |
| `session attach --list` | List running `mpbt-` tmux sessions and the agent-bus board in one call. |
| `session run <release> [--client claude\|opencode\|shell] [--name <id>] [--tier <tier>] [--supervisor <name>] [--permission-mode <mode>] [-- <args>]` | Launch a detached tmux session for an agent/CLI and post the initial heartbeat (replaces the legacy `agent-run` helper). Pass `--print` to emit the shell-evaluable launch variables (`STARFLEET_SHIP_ID`, `SESSION`, `RELEASE_FULL`, `CLIENT`, `INNER_CMD`) instead. |
| `session stop <id\|session>` | Kill a tmux session, clear its agent-bus heartbeat, and release its ship name (replaces the legacy `agent-run --stop` invocation). |
