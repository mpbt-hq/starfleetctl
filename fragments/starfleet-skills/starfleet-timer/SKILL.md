---
name: starfleet-timer
description: "starfleetctl timers — scheduled/interval/cron fleet directives, polling instead of blocking watch-loops. Load when setting a timer for a later check/reminder, or managing timers."
---

# starfleet-timer — fleet scheduling

Set **timers** instead of blocking a session with `sleep`/watch-loops (e.g. waiting for a
GitHub Actions run): the timer fires a comms directive into the fleet at the scheduled time,
and the poller injects it into the owning ship's next turn — no background process, no
blocking, survives a session restart.
