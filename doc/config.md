...
| [`models.yaml`](#modelsyaml-legacy) | legacy model registry (no longer used) |
...
The model catalog for the web UI dropdown is **no longer** a static file — it
is queried live from the configured model-proxy providers (upstream `/v1/models`
endpoints, enriched with label/context/caps) and served via `GET /api/models`.
The `starfleetctl models` subcommand and the `gen-models-yaml` script were
removed.

An older `.starfleet-ai/conf/models.yaml` left behind by previous versions is
